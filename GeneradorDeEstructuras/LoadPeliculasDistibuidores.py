...
#uses the id from the table Peliculas
#the table Peliculas has 2 columns: id, titulo
...
with open('../data/titles.csv', 'r', encoding="utf8") as f:
    ...
    for row in reader:
        ...
        c.execute("SELECT distributors FROM PeliculasDistibuidores WHERE title = ?", (row[1],))
        distributors = c.fetchone()
        #gets the id from the table Peliculas
        c.execute("SELECT id FROM Peliculas WHERE title = ?", (row[1],))
        idPelicula = c.fetchone()
        
        #if the title is not in the table Peliculas
        if idPelicula is None:
            print("title not found: " + str(row[1]) + " " + str(row[2]) + str(count))
            continue

        #sets row[1] to the id from the table Peliculas
        row[1] = idPelicula[0]

        #if the movie is not in the table
        if distributors is None:
            #insert the movie and the distributor
            c.execute("INSERT INTO PeliculasDistibuidores VALUES (?, ?)", (row[1], row[2]))
            ...
            if distributors[0].find(row[2]) == -1:
                ...
                #insert the movie and the distributor
                c.execute("UPDATE PeliculasDistibuidores SET distributors = ? WHERE title = ?", (distributors, row[1]))
        conn.commit()
